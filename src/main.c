...
static int broadcaster_message_handler(struct lws *wsi, unsigned char *msg, size_t len) {
    ...
    switch (type) {
        ...
        case BROADCASTER_MESSAGE:
            ...
            send_message(session_id, msg, len, 1);
            // debug - ping back
            message_len = sizeof("pong");
            message = malloc(message_len);
            memcpy(message, "pong", message_len);
            break;
        default:
...
static int viewer_message_handler(struct lws *wsi, unsigned char *msg, size_t len) {
    ...
    switch (type) {
        ...
        case VIEWER_MESSAGE:
            ...
            struct lws* broadcaster = find_broadcaster(session_id, wsi);
            if (!broadcaster) {
            ...
            message_len = sizeof("pong");
            message = malloc(message_len);
            memcpy(message, "pong", sizeof("pong"));
            break;
        default:
            message_len = strlen("Unknown message type: ") + 3;
            message = malloc(message_len);
...
static int callback_broadcaster(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_RECEIVE:
            ...
            // delete this block when have chance
            if (!msg && !remaining && lws_is_final_fragment(wsi)) {
